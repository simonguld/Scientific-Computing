...
def least_squares(A,b):                                     # A an mxn matrix
    ###
    # Transform an mxn matrix A to QR, where Q is orthogonal and R[0:n,:] is upper triangular.
    # if A has full column rank, then upper part of R, namely R[0:n,:] is invertible.
    # Since orthogonal transformations preserve norms, they preserve the (norm defined) solution to the least squares
    ...
    n = np.size(A[0, :])                                     # number of columns
    Q, R = householder_QR_factorization(A)                   # Obtain Q,R by householder factorization
...
# 1) The condition number function corresponding to the inf-norm is defined above.
# 2) Calculation of the condition number for (E-omega S) for the three listed values:

print("Project 1:")
print("\n")
print("Condition numbers for the given values of omega:")

for w in omega_values:
    print (f"Condition number ({w}) = ",f"{condition_number_inf(E-w*S):.2f}")

# 2) z is known to 8 significant digits, and so the rel. uncertainty in z is 0.5e-8. The relative uncertainty in the
# solution x for each omega is bounded by the product of the corresponding condition number and the relative uncertainty
# in z

z_rel_uncertainty = 0.5e-8
x_rel_uncertainty_bound = np.empty(n_omega)

for n in range(n_omega):
    x_rel_uncertainty_bound[n] = condition_number_inf(E-omega_values[n]*S)*z_rel_uncertainty

# and the number of guaranteed significant digits can be extracted by applying the negative base 10 logarithm to the
# absolute value of the upper bounds on the uncertainties in x. We thus have

significant_digits = - np.log10(np.abs(x_rel_uncertainty_bound))

print("\n")
print("Upper bounds on the relative uncertainties in x due to uncertainty in z:")

for n in range(n_omega):
    print(f"For omega = {omega_values[n] }: ","Rel. uncertainty (x) = ",f"{x_rel_uncertainty_bound[n]:.6e}",", Sigificant digits in x = ",f"{significant_digits[n]:.3}")
print("It follows that the large condition number generates a relatively large rel. error on x, especially around omega = 1.607, where we lose about 5.5 digits")


# Problem b:

# 1+2) The relative uncertainty in omega is given by 0.5e-3. This uncertainty perturbs the matrix E-omega*S, thus
# generating a forward error on x. Using the formula from the book, we can find an upper bound on this error, which
# we shall denote x_forward_error:

x_forward_error_bound = np.empty(n_omega)
forward_error_sig_digits = np.empty(n_omega)

print("\n")
print("Upper bounds on the relative forward uncertainties in x due to uncertainty in omega:")

for n in range(n_omega):
    x_forward_error_bound[n] = condition_number_inf(E-omega_values[n] * S) * norm_inf(omega_uncertainty * S) / norm_inf(E-omega_values[n] * S)
    forward_error_sig_digits[n] = - np.log10(np.abs(x_forward_error_bound[n]))

    print(f"For omega = {omega_values[n]} \u00B1 {omega_uncertainty:.0e}: ","forward rel. uncertainty (x) = ",
          f"{x_forward_error_bound[n]:.4e}",",   Sigificant digits = ",f"{forward_error_sig_digits[n]:.3}")
print("It follows that the large condition number generates a large forward error on x, especially around omega = 1.607, where we lose all"
      " significant digits.")
print("\n")

# Problem c: The three functions can be found at the top

# Problem d:

# We now calculate the polarizability for the frequencies listed. We will assume that the error on the
# polarizability is uniformly distributed around the real value, thus allowing us to express the result as
# the value of the polarizability corresponding to the unperturbed frequency plus/minus the uncertainty

for w in omega_values:
    p1 = polarizability(w+omega_uncertainty)    # Calculating the upper bound on the polarizability
    p2 = polarizability(w-omega_uncertainty)    # Calculating the lower bound on the polarizability

    p = polarizability(w)
    # calculate the uncertainty as the maximum distance between pol(omega) and pol at end points. This assumes that
    # the polarizability grows/decays uniformly on [w-omega_uncertainty,w] and on [w,w+omega_uncertainty], i.e. that all
    #extrema are corresponds to w-omega_uncertainty, w and w+omega_uncertainty
    uncertainty = np.max ([np.abs(p1-p), np.abs(p2-p)])

    abs_val = np.abs(p)
   # average = (p1+p2)/2                         # Average value taken to be the "true" value
    #uncertainty = abs(p1-p2)/2                  # Absolute value of absolute uncertainty
    relative_uncertainty = uncertainty / abs_val
    print (f"Polarizability ({w} \u00B1 {omega_uncertainty:.0e}) = ",f"{abs_val:.5}",u"\u00B1",f"{uncertainty:.3e}"
           , ",  Rel. uncertainty in alpha = ",f"{relative_uncertainty:.3e}")
print()
print("We note that the relative uncertainty in the polarizability is bounded by the forward relative error in x for,"
      " these frequencies, as expected. Note however, that all significant digits of the polarizability is"
      " lost for omega = 1.607")
print("In principle, both the error in z as well as the error in omega contributes to the error in x and thus in"
      " the polarizability. However, since the former is 3 orders of magnitude less than the latter, only the forward"
      " error contributes to the error in the polarizability")
...
N = 1000
omega_range = np.linspace(1.2,4,N)
polarizability_vectorized = np.vectorize(polarizability)
polarizability_values=polarizability_vectorized(omega_range)

omega_singularity = 1.60686978
# plotting the values, setting axis limits and labels

plt.figure()
plt.plot(omega_range,polarizability_values,'b-')
plt.xlim(1.2,4)
plt.ylim(-20,20)
plt.xlabel("Frequency",fontsize = 16)
plt.ylabel("Polarizability", fontsize = 16)
plt.title("Polarizability of water",fontsize=19)

print("\n")
print(f'We note that the polarizability has a singularity at omega_singularity = {omega_singularity} due to the very near singularity of'
      f' E-omega_singularity*S at this frequency. The condition number at this frequency has the value '
      f' {condition_number_inf(E-omega_singularity*S):.1e}. Specifically, it is due to the fact that the 3rd'
      f' diagonal element of the matrix at this frenquency is of the order of e-3, as is explained in more detail'
      f' in the rapport')
plt.show()

# Problem f: The householder_QR_factorization and least_squares functions can be found alongside the other functions
# functions at the top of the script.

# Problem g: In this problem, we are going to approximate the polarizability in the interval [1,omega_p] by even
# polynomials of degree 8 and 12 and compare their accuracy.
...
omega_p_index = 130                      #index corresponding to omega_p
...
x_p1 = least_squares(p_matrix1, polarizability_values[0:omega_p_index])   # coefficients for n=4 approximation
x_p2 = least_squares(p_matrix2, polarizability_values[0:omega_p_index])   # coefficients for n=6 approximation
...
# Problem h: In this final problem, we will approximate the polarizability with a rational function in order to
# correctly reproduce the singularity. At first, we will perform this approximation on [1.2,4].

# The rational function has the form given in the function rational_approximation, which can be found alongside the
# other functions. This function has been rewritten and approximated as a linear function in order to approximate its
# coefficients with the least squares method.
# The target matrix has the form [1,omega,omega^2,...,omega^m,-polarizability(omega)*omega,...
# -polarizability(omega)*omega^m]. In particular, we shall use m1 =2 and m2=4 and compare the accurary of these
# approximations. The right hand side of the matrix equation is polarizability(omega_range), and the least
# squares solution is given by x = (a0,a1,...,am,b1,...,bm)
...
x_q1 = least_squares(q_matrix1, polarizability_values)  #coefficients for approximation m=2
...
x_q2 = least_squares(q_matrix2, polarizability_values)  #repeat for m=4
...
x_q2 = least_squares(q_matrix2, polarizability_values)
