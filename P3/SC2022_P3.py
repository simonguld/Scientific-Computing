...
def golden_section_min_old(f,a,b,tolerance = 1e-3):
    ...
    calls, max_iterations = 2, 10000           # calls count the number of function calls
...
def golden_section_min_neww(f, a, b , tolerance = 1e-8):
    gr = (np.sqrt(5) + 1) / 2
    if 1:
        c = b - (b-a) / gr
        d = a + (b-a) / gr
...
def golden_section_min_new(f, a, b, tolerance = 1e-8):
...
#convergence for n=2,n=7 usual n=20 min with norm_grad>1, k<50 with alphamax = 0.2, tol = 1e-6, golden_tol =1e_3,
#alpha = 0.4 and alpha_max = 0.3 for k>50
#--> change alpha_max = 0.3 and get convergence for n=3

#convergence for 2 and 5: k<10 with alphamax = 0.3, k>10 with alphamax=0.7. grad>1.0 and alpha = 0.5. tol = 1e-8
# convergence for 2 and 4: k<5 with alphamax = 0.5. k>10 alphamax = 0.7. golden_tol = 1e-5, tol = 1e-8, grad > 0.2
def BFGS_old(f,gradf,X,tolerance = 1e-6, max_iterations = 1e4,linesearch=True): #takes flat vectors and functions
    ...
    for k in np.arange(max_iterations):
        ...
        if 0:
            ...
            BdY = B @ dY
            B += np.outer(dX, dX) / np.dot(dX, dX) - np.outer(BdY, BdY) / np.dot(dY, BdY)
        ...
        #konvergens for N=2,3 med grad>0.5, alpha_max=0.5, tol_golden_Sec = 1e-6
        #konv for N=4 for grad>1,  alpha_max = 0.8, tol_golden = 1e-3
        if linesearch and np.linalg.norm(grad) > 0.2:  # When close to convergence, linesearch in unnecessary
            ...
            if k < 5:
                alpha, steps = golden_section_min(lambda t: f(x+t*s), 0.01, 0.5, tolerance = 1e-8)
              #  alpha = scipy.optimize.golden(lambda t: f(x + t * s), brack=(0.01,0.5))
              #  calls += steps
                #print("k<50 alpha", alpha)
                x_new = x + alpha * s  # Update x
                grad_new = gradf(x_new)
            else:
                alpha, steps = golden_section_min(lambda t: f(x + t * s), 0.01, 0.7, tolerance=1e-8)
                #alpha = scipy.optimize.golden(lambda t: f(x + t * s), brack=(0.01,0.3))
               # calls += steps
               # print("k>50 alpha", alpha)
                x_new = x + alpha * s  # Update x
                grad_new = gradf(x_new)
        ...
        if 0:
            if np.dot(y,s)<0:
                print("y,s negative")
            if np.linalg.norm(y @ s) < 1e-8:
                print("snorm", np.linalg.norm(s))
                print (np.linalg.norm(np.outer(y,y)/(y@s)))
                print("sbs", np.linalg.norm((B @ np.outer(s,s) @ B) / (s @ B @ s)))
                print("BssB", np.linalg.norm((B @ np.outer(s,s) @ B)) )
                print("snorm", np.linalg.norm(s))
                print("normB", np.linalg.norm(B))

        if BFSGi:
            BdY = B @ y
            B += np.outer(s, s) / np.dot(s, s) - np.outer(BdY, BdY) / np.dot(y, BdY)
        else:
...
# for the math.
def BFGS(f, gradf, X0, tolerance=1e-6, max_iterations=10000, linesearch=True):
    BFGSi = True
    n_reset = 100 #n=75 for convergence of n=9
    n = len(X0) // 3
    B = np.eye(3 * n)  # Broyden approximation to Hessian
    ...
    for i in range(max_iterations):
        ...
        if (np.sqrt(np.dot(F0, F0)) < tolerance):
            converged = True
            break
        ...
        f_gamma = lambda t: f(X0 + t * dX)  # Restriction of f to line in step-direction

        alpha, n_calls = golden_section_min(f_gamma, 0, 1, tolerance=0.001)
       # grad_new = F0.astype('float')
        #alpha, n_calls, grad_new = wolfe_linesearch(f, gradf, X0, dX, F0)
        N_calls += n_calls + 1
        ...
        if BFGSi:
            # 1. Inverse BFGS update step
            BdY = B @ dY
            B += np.outer(dX, dX) / denominator - np.outer(BdY, BdY) / np.dot(dY, BdY)
           # print("norm x", np.linalg.norm(X0))
            #print("norm dx, norm dy, dot", np.linalg.norm(dX), np.linalg.norm(dY), np.dot(dX,dY))
            # 2. Compute quasi-Newton step with inverse scheme
