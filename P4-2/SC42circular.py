...
def circular_explicit_euler_solve(dy, P0, Q0, t_interval, dt):
...
def dy2(P, Q, dx, Dq, Dp, C, K):
    ...
    dP[index, 1:N - 1] = Dp / dx ** 2 * (P[(index + 1)%M, 1:N - 1] + P[(index - 1)%M, 1:N - 1] + P[1:N - 1, (index+1)%M]
                        + P[1:N - 1, (index-1)%M]) - (K + 1 + 4 * Dp / dx ** 2 - Q[index, 1:N - 1] * P[index, 1:N - 1]) \
                        * P[index, 1:N - 1] + C
    # Calculate dQ

    dQ[index, 1:N - 1] = Dq / dx ** 2 * ( Q[(index + 1)%M, 1:N - 1] + Q[(index - 1)%M, 1:N - 1] + Q[1:N - 1, (index+1)%M]
                        + Q[1:N - 1, (index-1)%M] - 4 * Q[index, 1:N - 1]) \
                         + (K - Q[index, 1:N - 1] * P[index, 1:N - 1]) * P[index, 1:N - 1]
...
def main():
    ...
    for K in Kval:
        ...
        euler_implicit, euler_explicit, RK4, crank_nicolson, euler_explicit_circ = False, True, False, False,True
        ...
        if euler_explicit_circ:
            time_initial = time.time()

            P, Q = explicit_euler_solve(lambda P, Q: dy2(P, Q, dx, Dq, Dp, C, K), P0, Q0, [0, T], dt)
            time_end = time.time()
            print(f' Runtime explicit for K = {K}: ', time_end - time_initial)
